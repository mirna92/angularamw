...
@Component({
  selector: 'app-updatoutlaytype',
  templateUrl: './updatoutlaytype.component.html',
  styleUrls: ['./updatoutlaytype.component.css']
})
export class UpdatoutlaytypeComponent implements OnInit {
  id!: string;
...
activeRequests:any[] = [];
  ...
  submit = async () => {
    this.loading=true;
    try {
      await this.usersService.editoutlaytype(this.id,this.form.value.nameFormControl,this.form.value.descriptionFormControl);
